...
import '../generated/google/firestore/v1/firestore.pbgrpc.dart';
...
class _FirestoreGatewayStreamCache {
  ...
  _FirestoreGatewayStreamCache(
      {this.onDone, required this.userInfo, this.onError}) {
...
class FirestoreGateway {
  ...
  /// This should allow to get stream as a result for any query
  /// against database. It is similar to [streamCollection] but for queries.
  ...
  void _setupClient() {
    ...
    // Enables grpc-web
    // https://github.com/grpc/grpc-dart/issues/263
  ...
  Stream<List<Document>> _mapCollectionStream(
    ...
    return listenRequestStream.stream.where((response) {
      return (response.hasTargetChange() &&
              response.targetChange.targetChangeType ==
